...
import activities.banking.DepositAll;
import activities.banking.DepositAllBanking;
import activities.banking.DepositAllDepositBox;
...
public class MiningActivity extends Activity {
	...
	private final List<String> cutGems = Arrays.asList("Opal", "Jade", "Red topaz", "Sapphire", "Emerald", "Ruby",
			"Diamond", "Dragonstone", "Onyx");
	private final List<String> uncutGems = Arrays.asList("Uncut opal", "Uncut jade", "Uncut red topaz",
			"Uncut sapphire", "Uncut emerald", "Uncut ruby", "Uncut diamond", "Uncut dragonstone", "Uncut onyx");
	...
	public void onStart() throws InterruptedException {
		if (mine == Mine.RIMMINGTON) {
			depositAll = new DepositAllDepositBox();
			depositAll.exchangeContext(getBot());
		} else {
			depositAll = new DepositAllBanking();
			depositAll.exchangeContext(getBot());
		}
		...
		if (pickaxeBanking.getCurrentTool() != null) {
			depositAll.setExceptItems(pickaxeBanking.getCurrentTool().getName());
		}
	...
	public void runActivity() throws InterruptedException {
		if (shouldBank()) {
			...
			getBank().close();
		} else if (pickaxeBanking.getCurrentTool().canEquip(getSkills())
				&& !getEquipment().isWearingItem(EquipmentSlot.WEAPON, pickaxeBanking.getCurrentTool().name)) {
	...
	protected boolean inventoryContainsNonMiningItem() {
		return getInventory().contains(item -> {
			...
			return !item.getName().contains(rock.ORE) && !cutGems.contains(item.getName())
